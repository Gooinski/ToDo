...
#Listbox with all tasks and scrollbar
...
#Adding itmes to the Listbox
with open('ToDo_Text.txt', 'r+') as tasks_list:
    for task in tasks_list:
        tasks.insert(END, task)
    tasks_list.close()

#Creating Entry widget for adding items
...
def add_item(entry: Entry, listbox: Listbox):
    ...
    with open('Todo_Text.txt', 'a') as tasks_list_file:
        tasks_list_file.write(f'\n{new_task}')

def delete_item(listbox: Listbox):
    listbox.delete(ACTIVE)
    with open('ToDo_Text.txt', 'r+') as tasks_list_file:
